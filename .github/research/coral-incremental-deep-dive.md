...
The idea: Instead of re-running the full `bar1 JOIN bar2` from scratch (which scans
everything), we only need to compute the **new join rows** caused by new data. Iceberg
tracks every write as a "snapshot" (like a git commit for tables). We use that to figure
out what's new.

There are **4 conceptual phases** in this Spark job:

#### Phase A: Set up Spark with Iceberg support

Nothing Coral-specific here — just standard Iceberg config so Spark can do time-travel queries.
...
#### Phase B: For each source table, figure out "what's new" using Iceberg snapshots

Every Iceberg table has a hidden `<table>.snapshots` metadata table that lists every
committed snapshot (think: every time data was written). We grab the **two most recent
snapshot IDs** — the "before" and "after" — to define our delta window.

```
Timeline of bar1:
  ┌──────────┐     ┌──────────┐     ┌──────────┐
  │ snap 100 │────▶│ snap 200 │────▶│ snap 300 │  ◀── latest
  │ (old)    │     │ (prev)   │     │ (current)│
  └──────────┘     └──────────┘     └──────────┘
                    ▲ start_snapshot   ▲ end_snapshot
                    
  "default_bar1"       = all rows as of snap 200 (the "before" state)
  "default_bar1_delta" = only rows ADDED between snap 200 → 300 (the new data)
```

```scala
// --- TABLE: bar1 ---

// Step B1: Read the snapshot history for bar1
val snapshot_df = spark.read.format("iceberg").load("bar1.snapshots")
snapshot_df.createOrReplaceTempView("snapshot_temp_table")

// Step B2: Grab the 2 most recent snapshot IDs
//   - snap_ids(0) = latest snapshot  (end)
//   - snap_ids(1) = previous snapshot (start)
val snap_ids = spark.sql("SELECT snapshot_id FROM snapshot_temp_table ORDER BY committed_at DESC LIMIT 2")
val start_snapshot_id = snap_ids.collect()(1)(0).toString  // previous
val end_snapshot_id = snap_ids.collect()(0)(0).toString    // latest

// Step B3: Load bar1 as it was BEFORE the new data arrived
//   This is the "old" complete state of bar1, needed because Coral's
//   rewritten query joins old-bar1 with new-bar2 (and vice versa)
val df = spark.read.format("iceberg").option("snapshot-id", start_snapshot_id).load("bar1")
df.createOrReplaceTempView("default_bar1")

// Step B4: Load ONLY the new rows added between the two snapshots
//   Iceberg's incremental read: returns just the data files added
//   after start_snapshot_id up to end_snapshot_id
...
At this point we have **4 temp views** registered in Spark SQL:

| Temp View | What it contains |
|-----------|-----------------|
| `default_bar1` | All rows in bar1 as of the **previous** snapshot (the "old" state) |
| `default_bar1_delta` | Only the **new rows** added to bar1 since last snapshot |
| `default_bar2` | All rows in bar2 as of the **previous** snapshot |
| `default_bar2_delta` | Only the **new rows** added to bar2 since last snapshot |

#### Phase C: Run the Coral-rewritten incremental query

This is the query Coral produced in Step 1. Instead of a full `bar1 JOIN bar2`, it
computes only the **new join results** using three terms (the standard delta rule for joins):

```
New join rows = (old_bar1 ⋈ new_bar2)       -- existing bar1 rows matching new bar2 rows
              ∪ (new_bar1 ⋈ old_bar2)       -- new bar1 rows matching existing bar2 rows  
              ∪ (new_bar1 ⋈ new_bar2)       -- new bar1 rows matching new bar2 rows
```
...
#### Phase D: Append the new results to the output table

The incremental results are **appended** to the existing output table. This is the
"merge" step — we're adding only the new join rows, not replacing the whole table.
# Coral Incremental View Maintenance: Deep Dive Research
...
#### How State Is Tracked Between Runs

**Coral itself stores no state.** It doesn't remember which snapshot it last processed.

State tracking relies entirely on **Iceberg's snapshot metadata** + a simple heuristic
in the generated code: **always use the 2 most recent snapshots** (`ORDER BY committed_at
DESC LIMIT 2`). This means:

```
Run 1: snapshots [100, 200] exist → delta = rows added between 100→200
Run 2: snapshots [100, 200, 300] exist → delta = rows added between 200→300
Run 3: snapshots [100, 200, 300, 400] exist → delta = rows added between 300→400
```

**⚠️ Important limitations of this approach:**

- If **multiple snapshots** are created between runs, only the **last two** are used — intermediate snapshots are skipped, and their data is lost from the incremental computation.
- There is **no bookmark or watermark** stored anywhere. If the job fails mid-run, there's no resume — the next run just picks up the latest two snapshots again.
- The dbt README notes this is hardcoded and "this logic will change to use Jasper in the future"[^5], suggesting a more robust snapshot tracking mechanism was planned but not yet implemented.
